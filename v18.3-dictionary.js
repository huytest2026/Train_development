 ...
 *  - Adds curated IPA when V17's result has no reliable IPA.
 *  - Protects the curated IPA while V17 finishes async rendering.
 */
(function () {
  'use strict';
  ...
  // Curated IPA for common exact forms. When UK/US differ, both are shown.
  // V18.3 uses this only when the V17 result does not already provide IPA.
  const IPA = Object.freeze({
    child: 'UK /tʃaɪld/ · US /tʃaɪld/',
    children: 'UK /ˈtʃɪldrən/ · US /ˈtʃɪldrən/',
    work: 'UK /wɜːk/ · US /wɝːk/',
    beautiful: '/ˈbjuːtɪfəl/',
    advice: '/ədˈvaɪs/',
    postponed: 'UK /pəˈspəʊnd/ · US /pəˈspoʊnd/',
    postpone: 'UK /pəˈspəʊn/ · US /pəˈspoʊn/',
    small: '/smɔːl/',
    concentrate: 'UK /ˈkɒnsəntreɪt/ · US /ˈkɑːnsəntreɪt/',
    beach: '/biːtʃ/',
    people: '/ˈpiːpəl/',
    person: '/ˈpɜːrsən/',
    men: '/men/',
  ...
  function patchRenderedIPA(word) {
    const root = getResultBox();
    if (!root) return false;

    const exact = norm(word);
    const ipa = IPA[exact];
    if (!exact || !ipa) return false;

    let box = root.querySelector('.v17-ipa');
    if (!box) {
      const headword = root.querySelector('.v17-headword');
      const vi = root.querySelector('.v17-vi');
      box = document.createElement('div');
      box.className = 'v17-ipa v183-ipa';
      box.innerHTML = '🔤 IPA: <span></span>';
      const anchor = headword || vi;
      if (anchor) anchor.insertAdjacentElement('afterend', box);
      else root.prepend(box);
    }

    const span = box.querySelector('span');
    if (span) span.textContent = ipa;
    box.dataset.v183IpaExact = exact;
    box.dataset.v183IpaVersion = VERSION;
    root.dataset.v183Ipa = ipa;
    return true;
  }
  ...
  async function patchCurrent(word) {
    ...
    const ipa = IPA[exact] || '';
    if (!translation && !ipa) return;

    if (norm(input.value) !== exact) return;

    // V17 can render from cache/shard/online at different times.
    for (let i = 0; i < 60; i++) {
      const viDone = translation ? patchRenderedResult(exact, translation) : true;
      const ipaDone = ipa ? patchRenderedIPA(exact) : true;
      if (viDone && ipaDone) return;
  ...
  function installObserver() {
    ...
    const observer = new MutationObserver(function () {
      ...
      if (IPA[exact]) {
        const ipaBox = root.querySelector('.v17-ipa');
        if (!ipaBox ||
            ipaBox.dataset.v183IpaExact !== exact ||
            ipaBox.dataset.v183IpaVersion !== VERSION) {
          patchRenderedIPA(exact);
        }
      }
    });
